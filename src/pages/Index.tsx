          ...
          {videos.map((video, index) => (
            ...
            >
              ...
              <video
                ...
                muted
                playsInline
                className="w-full max-w-[200px] sm:max-w-[180px] md:max-w-[220px] lg:max-w-[280px] h-[280px] sm:h-[260px] md:h-[320px] lg:h-[420px] object-cover rounded-2xl border border-border transition-all duration-500 group-hover:scale-105 group-hover:shadow-xl group-hover:shadow-primary/30 group-hover:border-primary/50"
              />
            </div>
          ))}
        </div>
...
const PlatformsContent = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-4 md:px-8 overflow-y-auto py-6">
      <h2 className="text-xl md:text-2xl font-bold text-center mb-2">{t('platforms.title')}</h2>
      <p className="text-muted-foreground text-center mb-6 md:mb-12 max-w-md text-sm md:text-base">{t('platforms.subtitle')}</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-16">
        {platforms.map((platform) => (
          <div key={platform.name} className="flex flex-col items-center gap-3 md:gap-6">
            <span className="text-lg md:text-2xl font-bold">{platform.name}</span>
            <div 
              className="w-24 h-24 md:w-40 md:h-40 rounded-2xl md:rounded-3xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-300"
              style={{ backgroundColor: platform.color }}
            >
              <svg viewBox="0 0 24 24" className="w-14 h-14 md:w-24 md:h-24" fill="white">
    ...
    <div className="h-full overflow-y-auto px-6 py-6">
      ...
      <section className="mb-10 animate-fade-in">
        ...
        <div className="flex justify-center mb-6 overflow-x-auto px-2">
          <div className="inline-flex items-center gap-1 bg-secondary/50 dark:bg-[#0f0f1a] rounded-full p-1 md:p-1.5 border border-border/30">
            {campaignDaysOptions.map((days) => (
              <button
                key={days}
                onClick={() => setSelectedCampaignDays(selectedCampaignDays === days ? null : days)}
                className={`px-2 md:px-4 py-1.5 md:py-2 text-xs md:text-sm rounded-full transition-all duration-300 whitespace-nowrap ${
          ...
          <div className="inline-flex items-center gap-1 bg-secondary/50 dark:bg-[#0f0f1a] rounded-full p-1 md:p-1.5 border border-border/30">
            {subscriptionDaysOptions.map((days) => (
              <button
                ...
                className={`px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm rounded-full transition-all duration-300 whitespace-nowrap ${
...
const ContactContent = () => {
  ...
  const socialLinks = [
    { name: 'Instagram', url: 'https://www.instagram.com/autoboom_g?igsh=MXIzenFqMGQ1MWNvdg%3D%3D', icon: (
      <svg viewBox="0 0 24 24" className="w-8 h-8" fill="currentColor">
        <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z"/>
      </svg>
    )},
    { name: 'Facebook', url: 'https://www.facebook.com/profile.php?id=61581622864039', icon: (
      <svg viewBox="0 0 24 24" className="w-8 h-8" fill="currentColor">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
      </svg>
    )},
    { name: 'TikTok', url: 'https://www.tiktok.com/@autoboom_g', icon: (
      <svg viewBox="0 0 24 24" className="w-8 h-8" fill="currentColor">
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-8 animate-fade-in">
      <h2 className="text-2xl font-bold mb-4 text-foreground">{t('contact.title')}</h2>
      
      {/* Book a call text */}
      <p className="text-muted-foreground text-center mb-6">{t('contact.bookCall')}</p>
      
      {/* CTA Button */}
      <button 
        className="px-8 py-3 bg-primary text-primary-foreground rounded-full font-medium hover:opacity-90 transition-opacity mb-8"
